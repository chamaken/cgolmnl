...
var _ = Describe("callback", func() {
	...
	var (
		...
		nlmsghdr_pid2,
		nlmsghdr_seq2,
		nlmsghdr_intr *NlmsgBuf
	)

	BeforeEach(func() {
		nlmsghdr_noop = NewNlmsgBuf(16)
		...
		nlmsghdr_overrun = NewNlmsgBuf(16)
		...
		nlmsghdr_error = (*NlmsgBuf)(&b2)
		nlmsghdr_error.SetLen(uint32(len(b2)))

		typeFF_msg := NewNlmsgBuf(16)
		...
		nlmsghdr_typeFF = (*NlmsgBuf)(&b3)

		pid2_msg := NewNlmsgBuf(16)
		pid2_msg.SetLen(16)
		...
		nlmsghdr_pid2 = (*NlmsgBuf)(&b4)

		seq2_msg := NewNlmsgBuf(16)
		seq2_msg.SetLen(16)
		...
		nlmsghdr_seq2 = (*NlmsgBuf)(&b5)

		intr_msg := NewNlmsgBuf(16)
		intr_msg.SetLen(16)
		...
		nlmsghdr_intr = (*NlmsgBuf)(&b6)
	})

	Context("CbRun", func() {
		cb := func(nlh *Nlmsg, data interface{}) (int, syscall.Errno) {
			if data != nil {
				l := data.(*[]uint16)
				*l = append(*l, nlh.Type)
			}
	...
	Context("CbRun2", func() {
		...
		It("should return (default) MNL_CB_OK", func() {
			ctl_cbs2 := []MnlCb{nil, nil, ctl_cbs[NLMSG_ERROR], ctl_cbs[NLMSG_DONE], ctl_cbs[NLMSG_DONE]}
			ret, err := CbRun2(([]byte)(*nlmsghdr_noop), 1, 1, nil, nil, ctl_cbs2)
			Expect(err).To(BeNil())
			Expect(ret).To(Equal(MNL_CB_OK))
		})
		It("should return MNL_CB_STOP", func() {
			ret, err := CbRun2(([]byte)(*nlmsghdr_done), 1, 1, nil, nil, ctl_cbs)
			Expect(err).To(BeNil())
			Expect(ret).To(Equal(MNL_CB_STOP))
		})
		It("should return ENOBUFS", func() {
			ret, err := CbRun2(([]byte)(*nlmsghdr_overrun), 1, 1, nil, nil, ctl_cbs)
			Expect(err).To(Equal(syscall.ENOBUFS))
			Expect(ret).To(Equal(MNL_CB_ERROR))
		})
		It("should return EPERM", func() {
			ret, err := CbRun2(([]byte)(*nlmsghdr_error), 1, 1, nil, nil, ctl_cbs)

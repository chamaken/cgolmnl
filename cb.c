...
{
	int i, rc;
        mnl_cb_t *ctl_cbs = calloc(sizeof(mnl_cb_t), ctl_cbs_len); /* or alloca? */

        if (ctl_cbs == NULL)
                return MNL_CB_ERROR;

        for (i = 0; i < ctl_cbs_len; i++)
                ctl_cbs[i] = (mnl_cb_t)GoCtlCb;

	rc = mnl_cb_run2(buf, numbytes, (unsigned int)seq, (unsigned int)portid,
                         (mnl_cb_t)GoCb, (void *)data, ctl_cbs, ctl_cbs_len);
        free(ctl_cbs);
